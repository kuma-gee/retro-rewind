...
func do_move(node: Node2D, motion: Vector2i, finish_fn: Callable):
	var pos = local_to_map(node.position)
	if motion.x != 0:
		var dest = pos + Vector2i(motion.x, 0)
		if not _is_wall(dest):
			return _move(node, dest, finish_fn)
...
func _move(node: Node2D, dest: Vector2i, finish_fn: Callable):
	var rect = get_used_rect()
	var pos = map_to_local(dest)
	var tw = create_tween()
	tw.tween_property(node, "position", pos, 0.2)
	tw.finished.connect(func():
		if dest.x >= rect.size.x:
			...
			tw2.tween_property(node, "position", other_pos, 0.2)
